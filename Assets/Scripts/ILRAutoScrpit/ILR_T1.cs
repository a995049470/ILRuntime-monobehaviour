...
public class ILR_T1 : ILRMono
{
	[SerializeField]
	protected Data_ILR_T1 m_Data;
    protected static string m_TypeName = "T1";
    protected static bool bIsDataEmpty = false;
    ...
    //protected static Dictionary<string, Data_ILR_T1> m_DataDic = new Dictionary<string, Data_ILR_T1>();
    private void Awake()
    {
    ...
    {	
		m_Data.t2.Init();
		SetValue("t2", m_Data.t2.Self);
		SetValue("go", m_Data.go);
...
//        var data = JsonUtility.FromJson<Data_ILR_T1>(jsonStr);
//        m_DataDic[key] = data;
//    }
...
//         jsonStr = RecoverInstanceID(jsonStr);
//         m_Data = JsonUtility.FromJson<Data_ILR_T1>(jsonStr);
// #else
// #endif
...
#if ILRuntime
        protected static MethodInfo _Test;
#else
        protected static IMethod m_Test;
#endif    
    public void Test(System.String arg1)
    {
        param1[0] = arg1;  
#if ILRuntime
        if(_Test == null)
        {
                _Test = _Type.GetMethod("Test", BindingFlags.FlattenHierarchy | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
        }
        
         _Test.Invoke(m_Self, param1);
#else
        if(m_Test == null)
        {
                m_Test = m_Type.GetMethod("Test", 1);
        }
        m_AppDomain.Invoke(m_Test, m_Self, param1);
#endif
    }
